...
const SHIFT_STYLE: Record<ShiftStatus, { text: string; bg: string }> = {
  EVIDENCED: { text: "text-emerald-400", bg: "bg-emerald-500/10" },
  DIRECTIONAL: { text: "text-amber-400", bg: "bg-amber-500/10" },
  HYPOTHESIS: { text: "text-cyan-400", bg: "bg-cyan-500/10" },
};
    ...
    >
      <div className="fade-up">
        ...
        <h2 className="mt-3 max-w-2xl text-2xl font-semibold tracking-[-0.02em] text-zinc-100 sm:text-3xl">
          One run is a demo.{" "}
          <span className="text-zinc-500">A library is a moat.</span>
        </h2>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_1.3fr]">
          ...
          <div className="card flex flex-col overflow-hidden">
            <div className="flex items-center justify-between border-b border-line px-5 py-3">
              ...
              <span className="mono-num text-[10px] text-zinc-600">
                valueshift.pattern/v1
              </span>
            </div>
            <pre className="flex-1 overflow-x-auto p-5 font-mono text-[11px] leading-[1.7] text-zinc-400">
            ...
            <div className="border-t border-line p-4">
              <div className="flex flex-wrap gap-1.5">
                {["no names", "no dollars", "coarse bands", "normalized ratios"].map(
                  (chip) => (
                    <span
                      key={chip}
                      className="flex items-center gap-1 rounded-md bg-white/[0.05] px-2 py-1 text-[10px] font-medium tracking-wide text-zinc-400"
                    >
                      <Lock size={9} />
                      {chip}
                    </span>
              ...
              <button
                ...
                className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-cyan-500/40 bg-cyan-500/10 px-4 py-2.5 text-[13px] font-semibold text-cyan-200 transition-colors hover:bg-cyan-500/20"
              >
                <Download size={14} />
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-line/60 last:border-0 ${
                          isRun ? "bg-cyan-500/[0.07]" : ""
                        }`}
                      >
                        <td
                          className={`mono-num px-4 py-2.5 text-[11px] ${
                            isRun ? "font-bold text-cyan-300" : "text-zinc-500"
                          }`}
                        >
                          {label}
                        </td>
                        ...
                        <td
                          className={`mono-num px-4 py-2.5 text-[12px] font-semibold ${
                        ...
                        <td className="mono-num px-4 py-2.5 text-[12px] text-zinc-400">
        ...
        <div className="card mt-5 p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {findings.map((f) => (
              <div
                key={f}
                className="rounded-xl border border-line bg-canvas/50 p-4 text-[12.5px] leading-relaxed text-zinc-400"
              >
                {f}
              </div>
            ))}
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {ROADMAP.map((m, i) => (
              <div key={m.horizon} className="card relative p-5">
                <span className="mono-num text-[11px] font-semibold text-cyan-400/80">
                  {String(i + 1).padStart(2, "0")} · {m.horizon}
                </span>
                <h3 className="mt-2.5 text-[13.5px] font-semibold text-zinc-200">
                  {m.title}
                </h3>
            ...
            {STRUCTURAL_SHIFTS.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`mono-num shrink-0 rounded-md px-2 py-1 text-[9px] font-bold tracking-[0.12em] ${style.text} ${style.bg}`}
                  >
                    {s.status}
                  </span>
                </div>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          <div className="card p-6">
            <div className="flex items-center gap-2 text-zinc-300">
              <Lock size={14} />
              <span className="micro-label !text-zinc-400">The restraint</span>
            </div>
          ...
          <div className="card border-cyan-500/20 bg-cyan-500/[0.04] p-6">
            ...
            <dl className="mt-4 space-y-3.5 text-[12.5px] leading-relaxed">
              <div>
