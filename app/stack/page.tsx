...
const HELD_STYLE: Record<Held, { bg: string; text: string }> = {
  sold: { bg: "bg-emerald-500/10", text: "text-emerald-400" },
  free: { bg: "bg-amber-500/10", text: "text-amber-400" },
  absent: { bg: "bg-cyan-500/10", text: "text-cyan-400" },
};

const STANDING_STYLE: Record<Standing, { hex: string; bg: string; text: string }> = {
  owned: { hex: "#10b981", bg: "bg-emerald-500/10", text: "text-emerald-400" },
  partial: { hex: "#f59e0b", bg: "bg-amber-500/10", text: "text-amber-400" },
  open: { hex: "#06b6d4", bg: "bg-cyan-500/10", text: "text-cyan-400" },
};
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 pb-20">
                  ...
                  <div className="grid gap-5 lg:grid-cols-[1.15fr_1fr]">
                    <div>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-baseline gap-3">
                          <span className="mono-num text-[11px] font-semibold text-zinc-700">
                            {l.n}
                          </span>
                        ...
                        <span
                          className={`mono-num inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1 text-[9px] font-bold tracking-[0.12em] ${s.bg} ${s.text}`}
                        >
                          {l.standing === "owned" ? (
                            <Check size={9} />
                          ) : l.needsScale ? (
                      ...
                      <p className="mt-3 border-l-2 border-line pl-3.5 text-[12px] leading-relaxed text-zinc-500">
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-baseline gap-3">
                        ...
                        <h3 className="text-[14.5px] font-semibold text-zinc-100">
                          {o.name}
                        </h3>
                      </div>
                      <span
                        className={`mono-num shrink-0 rounded-md px-2 py-1 text-[9px] font-bold tracking-[0.12em] ${h.bg} ${h.text}`}
                      >
                    ...
                    <p className="mono-num mt-3 text-[12px] text-cyan-300">{o.range}</p>
                  </div>
                  <div className="rounded-xl border border-line bg-canvas/50 p-4">
                    <p className="text-[12.5px] leading-relaxed text-zinc-400">{o.note}</p>
                  </div>
                </div>
              );
            })}
        ...
        <section className="mt-14">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {EVIDENCE_GATED.map((e) => (
              <div
                key={e.name}
                className="card border-cyan-500/25 bg-cyan-500/[0.04] p-6"
              >
                <h3 className="text-[14px] font-semibold text-zinc-200">{e.name}</h3>
                <p className="mt-2.5 text-[12.5px] leading-relaxed text-zinc-400">
                  {e.why}
                </p>
              </div>
          ...
          <p className="mt-5 max-w-3xl rounded-xl border border-line bg-canvas/50 p-5 text-[13px] leading-relaxed text-zinc-300">
        ...
        <section className="mt-14">
          ...
          <div className="card mt-5 border-cyan-500/30 bg-cyan-500/[0.06] p-7">
            <p className="text-xl font-semibold leading-snug tracking-[-0.02em] text-zinc-100 sm:text-2xl">
              {FLOOR_FINDINGS[0].title}
            </p>
            <p className="mt-3 max-w-3xl text-[13px] leading-relaxed text-zinc-300">
            ...
            {FLOOR_CASES.map((c) => (
              <div
                ...
                className={`card grid gap-5 p-6 lg:grid-cols-[1fr_1.15fr] ${
                  c.exception ? "border-emerald-500/30 bg-emerald-500/[0.04]" : ""
                }`}
              >
                <div>
                  <div className="flex items-baseline gap-3">
                  ...
                  <div
                    className={`mt-3 overflow-x-auto rounded-lg border px-3.5 py-2.5 ${
                      c.exception
                        ? "border-emerald-500/30 bg-canvas/60"
                        : "border-line bg-canvas/60"
                    }`}
                  >
                    <code
                      className={`mono-num whitespace-nowrap text-[12px] ${
          ...
          <p className="mt-4 max-w-3xl text-[12.5px] leading-relaxed text-zinc-500">
            {FLOOR_WHY}
          </p>
          <p className="mt-4 max-w-3xl rounded-xl border border-cyan-500/25 bg-cyan-500/[0.05] p-5 text-[13px] leading-relaxed text-zinc-300">
            {FLOOR_CONTRAST}
          </p>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {FINDINGS.map((f) => (
              <div key={f.title} className="card p-6">
                <h3 className="text-[14px] font-semibold text-zinc-200">{f.title}</h3>
                <p className="mt-2.5 text-[12.5px] leading-relaxed text-zinc-400">
                  {f.body}
                </p>
              </div>
